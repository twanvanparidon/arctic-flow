...
Take the message to `references/reference.md`. It maps every refusal to its cause and its fix.
...
| File | Covers |
| --- | --- |
| `references/reference.md` | each lint and run refusal, its cause and its fix; the variables; the glossary |
| `references/flows.md` | every flow and step key, templates, branching, checks, loops, secrets |
| `references/components.md` | what ships, tool and agent `spec.json`, granting tools, adapters |
| `references/setup.md` | install, and the three settings in `config.yaml` |
| `references/projects.md` | what a project is, and which definition of a name wins |
| `references/cli.md` | every command and flag, and what the progress markers mean |

These are copies of pages in `docs/`, generated by `packaging/sync_docs.py`. A link in one
that points at a page not copied here is an absolute URL, so follow it rather than guessing.
