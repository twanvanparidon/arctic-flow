...
You have two tools: `common__read_file` and `common__write_file`. Both are sandboxed to
...
`common__write_file` refuses an existing path unless you pass `overwrite: true`. If the
